    ...
    return (
      <div className="fixed inset-0 bg-black/70 dark:bg-black/90 z-50 flex items-center justify-center p-4 md:p-8">
        <motion.div
          ...
          className="bg-white dark:bg-black/80 backdrop-blur-lg rounded-xl w-full max-w-4xl max-h-[90vh] overflow-hidden relative border border-indigo-500/30 dark:border-indigo-500/20"
        >
          <button
            onClick={() => setSelectedPath(null)}
            className="absolute p-2 top-2 right-2 text-gray-900 dark:text-white hover:text-indigo-300 dark:hover:text-indigo-500 flex items-center gap-2 z-10"
          >
            <X />
          </button>
          ...
          <div className="p-4 md:p-8 space-y-4 md:space-y-6 max-h-[60vh] overflow-y-auto scrollbar-thin scrollbar-thumb-indigo-500/30 scrollbar-thumb-indigo-400 dark:scrollbar-thumb-indigo-500/40 scrollbar-track-transparent hover:scrollbar-thumb-indigo-400">
            {pathSkills.map((skill) => (
              <motion.div
                ...
                className="relative"
              >
                <Card className="border border-indigo-500/30 dark:border-indigo-500/20 bg-white/10 dark:bg-black/15 backdrop-blur-lg overflow-hidden">
                  <div className="p-4 md:p-6 space-y-3 md:space-y-4">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="text-lg md:text-xl font-bold text-gray-900 dark:text-indigo-300">{skill.name}</h3>
                          ...
                          <Star
                            ...
                            className={`w-3 h-3 md:w-4 md:h-4 ${
                              i < skill.proficiency / 2
                                ? 'text-indigo-500 fill-indigo-500 dark:text-indigo-300 dark:fill-indigo-300'
                                : 'text-indigo-500/20 dark:text-indigo-500/20'
                            }`}
                          />
                        ))}
                      </div>
                      <Badge className="bg-indigo-500/20 dark:bg-indigo-500/20 text-indigo-500 dark:text-indigo-300 text-xs md:text-sm">
                        {skill.years}y exp
                      </Badge>
                    </div>
                  </div>
